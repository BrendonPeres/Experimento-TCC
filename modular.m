function [dadoModulado,bitsNaoPilotos] = modular(dado, piloto) %MatlabDoc:DPSK, PSK, PAM, QAM | Wikipedia:ASK, FSK, PSK, QAM
    ...
    for idx = 1:size(piloto.localizacao,2)
        if (piloto.metodo(idx) == "dpsk")
            dadoModulado(piloto.localizacao(idx)) = dpskmod(randi([0 (piloto.grau(idx) - 1)]), piloto.grau(idx)) * piloto.razaoPilotoDado(idx);
        elseif (piloto.metodo(idx) == "psk")
            dadoModulado(piloto.localizacao(idx)) = pskmod(randi([0 (piloto.grau(idx) - 1)]), piloto.grau(idx)) * piloto.razaoPilotoDado(idx);
        elseif (piloto.metodo(idx) == "pam")
            dadoModulado(piloto.localizacao(idx)) = pammod(randi([0 (piloto.grau(idx) - 1)]), piloto.grau(idx)) * piloto.razaoPilotoDado(idx);
        elseif (piloto.metodo(idx) == "qam")
            dadoModulado(piloto.localizacao(idx)) = qammod(randi([0 (piloto.grau(idx) - 1)]), piloto.grau(idx)) * piloto.razaoPilotoDado(idx);
        end
    end
